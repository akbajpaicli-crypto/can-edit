...
class GridIndex {
  ...
  constructor(gridDeg = 0.01) {
    this.grid = new Map();
    this.gridDeg = gridDeg;
  }
  private cell(lat: number, lon: number) {
    return `${Math.floor(lat / this.gridDeg)},${Math.floor(lon / this.gridDeg)}`;
  }
...
function findColumn(data: Array<Record<string, string>>, names: string[]): string | null {
  ...
  for (const name of names) {
    const found = headers.find(
      (h) => h.toLowerCase() === name.toLowerCase() || h.toLowerCase().replace(/[^a-z]/g, '') === name.toLowerCase()
    );
...
export async function analyzeData(
  ...
  if (!rtisLatCol || !rtisLonCol || !rtisTimeCol || !rtisSpeedCol)
    throw new Error('RTIS file missing critical columns');
  ...
  const masterLabelCol =
    findColumn(masterDataRaw, ['OHEMas', 'OHE', 'pole', 'Signal', 'Name', 'Station', 'Label', 'Asset']) || 'Location';
    ...
    data.forEach((row) => {
      ...
      if (bestP && bestDist <= threshold) {
        ...
        if (type === 'OHE') {
          ...
          if (speed! > limitApplied + 3) {
            status = 'violation';
          } else if (speed! > limitApplied) {
            status = 'warning';
          }
        }
      }
  ...
  const finalResults = processDataset(
    masterDataRaw,
    isOheMaster ? 'OHE' : 'Signal',
    masterLatCol,
    masterLonCol,
    masterLabelCol
  );
  ...
  const validSectionData =
    firstMatchedRtisIndex !== Infinity && lastMatchedRtisIndex !== -1
      ? rtisCleaned.filter((p) => p.idx >= firstMatchedRtisIndex && p.idx <= lastMatchedRtisIndex)
      : [];

  // --- STEP 7: BRAKE TESTS (Strict Circular Logic) ---
  ...
  if (validSectionData.length > 0) {
    ...
    if (entrySpeed > 15) {
      ...
      for (let i = 0; i < validSectionData.length - 1; i++) {
        ...
        if (p.speed > 15) {
          bftResult = { type: 'BFT', status: 'improper', startSpeed: p.speed, lowestSpeed: p.speed, dropAmount: 0, location: findNearestLocation(p.lat, p.lon), timestamp: p.timeStr, details: "Crossed 15 km/h without successful test" };
          bftFailed = true;
          break;
        }
        // Looking for Start point ~15 (14-15 allowed)
        if (p.speed >= 14 && p.speed <= 15) {
          let minSpeed = p.speed;
          // Look ahead 30 seconds for drop to ~10
          ...
          // Requirement: Drop to 10. (So lowest should be <= 11 to be safe)
          if (minSpeed <= 11) {
            bftResult = { type: 'BFT', status: 'proper', startSpeed: p.speed, lowestSpeed: minSpeed, dropAmount: p.speed - minSpeed, location: findNearestLocation(p.lat, p.lon), timestamp: p.timeStr, details: "Speed dropped to ~10 km/h" };
            break;
          }
        }
      }
      if (bftResult) brakeTests.push(bftResult);
      else if (!bftFailed) brakeTests.push({ type: 'BFT', status: 'not_performed', startSpeed: 0, lowestSpeed: 0, dropAmount: 0, location: "-", timestamp: "-", details: "Did not reach 15 km/h" });


      // --- BPT LOGIC ---
      // Passenger: 60-70 -> Drop 30-35
      // Goods: 40-50 -> Drop 20-25
      ...
      for (let i = 0; i < validSectionData.length - 1; i++) {
        const p = validSectionData[i];
        if (haversineMeters(startLoc.lat, startLoc.lon, p.lat, p.lon) > 15000) break; // First 15km
        ...
        if (p.speed >= minStart && p.speed <= maxStart) {
          ...
          if (drop >= reqDropMin) {
            bptResult = { type: 'BPT', status: 'proper', startSpeed: p.speed, lowestSpeed: minSpeed, dropAmount: drop, location: findNearestLocation(p.lat, p.lon), timestamp: p.timeStr, details: `Drop of ${drop} km/h` };
            break;
          }
        }
      }
      ...
      if (!bptResult) {
         // Check if they ignored it
         const opp = validSectionData.find(p => p.speed >= minStart);
         if(opp && haversineMeters(startLoc.lat, startLoc.lon, opp.lat, opp.lon) <= 15000) {
             brakeTests.push({ type: 'BPT', status: 'improper', startSpeed: opp.speed, lowestSpeed: opp.speed, dropAmount: 0, location: findNearestLocation(opp.lat, opp.lon), timestamp: opp.timeStr, details: "Speed reached but no braking detected" });
         } else {
             brakeTests.push({ type: 'BPT', status: 'not_performed', startSpeed: 0, lowestSpeed: 0, dropAmount: 0, location: "-", timestamp: "-", details: `Did not reach ${minStart} km/h` });
         }
      } else {
  ...
  stoppages.forEach((stop) => {
    ...
    // 2. Previous Signals
    const sigHistoryIdx = signalHistory.findIndex(
      (s) => s.location === stop.location && new Date(s.logging_time).getTime() <= new Date(stop.arrivalTime).getTime()
    );
      ...
      if (sigHistoryIdx > 0) {
        ...
        if ((prev1.speed_kmph || 0) > LIMIT_PREV1) {
          haltViolations.push({
            ...
            limit: LIMIT_PREV1,
            actualSpeed: Math.round(prev1.speed_kmph || 0),
      ...
      if (sigHistoryIdx > 1) {
        ...
        if ((prev2.speed_kmph || 0) > LIMIT_PREV2) {
          haltViolations.push({
            ...
            limit: LIMIT_PREV2,
            actualSpeed: Math.round(prev2.speed_kmph || 0),
